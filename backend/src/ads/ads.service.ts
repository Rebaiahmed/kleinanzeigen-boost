...
export class AdsService {
  ...
  async saveDraft(userId: string, adData: SaveDraftDto) {
    const db = this.firebaseService.firestore;
    // Images never reach the backend — they're stored client-side in IndexedDB,
    // keyed by this same draft id, so the frontend generates it upfront rather
    // than relying on the id this method would otherwise generate.
    const adId = adData.id || `draft_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
    const adRef = db.collection('users').doc(userId).collection('ads').doc(adId);

    // class-transformer instantiates `vinted` as a VintedDraftDto (needed for
    // @ValidateNested to run) — Firestore refuses to write objects with a
    // custom prototype, so flatten it back to a plain object before saving.
    const { vinted, ...restData } = adData;
